...
class AnimeService {
    ...
    public async setPopularAnime(): Promise<Anime[]> {
        ...
        const response = await axios.get<AnimeResponse>(
            'https://api.jikan.moe/v4/top/anime?type=tv&filter=bypopularity&limit=10&page=1'
        );
        if (response.data?.data) {
            ...
            this.saveLocalData([...animeListFromStorage, ...animeList]);
        }
        return animeList;
    }
    ...
    private loadLocalData(): Anime[] {
        try {
            ...
            if (storedData) {
                return JSON.parse(storedData) as Anime[];
            }
            return [];
        } catch (error) {
