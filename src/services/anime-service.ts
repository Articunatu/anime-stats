...
class AnimeService {
   ...
   public async setPopularAnime(totalEntries = 100): Promise<Anime[]> {
        ...
        const maxPages = Math.ceil(totalEntries / limitPerPage);
        
        for (let page = 1; page <= maxPages; page++) {
            const response = await axios.get<AnimeResponse>(
                `https://api.jikan.moe/v4/top/anime?type=tv&filter=bypopularity&limit=${limitPerPage}&page=${page}`
            );
            ...
            if (animeList.length >= totalEntries) {
                break;
            }
        }
